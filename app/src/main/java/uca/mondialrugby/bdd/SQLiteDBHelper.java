...
public class SQLiteDBHelper extends SQLiteOpenHelper {
	...
	private static final int DATABASE_VERSION = 50;      /* A incrémenter quand on modifie cette classe */
    ...
    /*
        - VUES -
        Voici un ensemble de vues qui permettent de retourner grace à la vue "CLASSEMENT",
        un classement des pays en les ordannant par ordre décroissant du nombre de victoires
        et en concaténant les équipes n'ayant encore jamais gagné
	 */
	
	/* Vue sur les matchs terminés */
	private static String VIEW_LISTE_FINIS = "" +
			"CREATE VIEW IF NOT EXISTS listfini AS  " +
			"SELECT  * " +
			"FROM Jouer " +
			"WHERE score != '';";
	
	/* Vue des gagnats des matchs */
	private static String VIEW_GAGNANTS = "" +
			"CREATE VIEW IF NOT EXISTS B AS " +
			"SELECT j1.pays, j1.ID_MATCH, j1.SCORE " +
			"FROM Jouer AS j1, Jouer AS j2 " +
			"WHERE j1.ID_MATCH = j2.ID_MATCH and j1.score > j2.score;";
	
	/* Vue sur la liste des pays n'ayant jamais gagnés */
	private static String VIEW_PERDANTS = "" +
			"CREATE VIEW IF NOT EXISTS N AS " +
			"SELECT * " +
			"FROM Equipe " +
			"WHERE pays NOT IN(SELECT pays FROM B);";
	
	/* Vue sur le classement des pays trié par le nombre de victoires */
	private static String VIEW_CLASSEMENT = "" +
			"CREATE VIEW IF NOT EXISTS CLASSEMENT AS" +
			" SELECT pays, count(pays) AS victoires" +
			" FROM B" +
			" GROUP BY pays" +
			" UNION ALL" +
			" SELECT pays, 0" +
	...
	public void onCreate(SQLiteDatabase db) {
		...
		// Création des vues pour le classement des équipes
		db.execSQL(VIEW_LISTE_FINIS);
		db.execSQL(VIEW_GAGNANTS);
		db.execSQL(VIEW_PERDANTS);
		db.execSQL(VIEW_CLASSEMENT);
